...
class TestMain(unittest.TestCase):
    ...
    @patch("autopkgserver.os.geteuid")
    @patch("autopkgserver.os.stat")
    @patch("autopkgserver.os.path.realpath")
    @patch("autopkgserver.os.path.abspath")
    @patch("autopkgserver.os.path.dirname")
    @patch("autopkgserver.sys.argv", ["/usr/local/bin/autopkgserver"])
    def test_main_checks_ownership(
        ...
        mock_abspath.return_value = "/usr/local/bin/autopkgserver"
        mock_realpath.return_value = "/usr/local/bin/autopkgserver"
    ...
    @patch("autopkgserver.os.path.dirname")
    @patch("autopkgserver.sys.argv", ["/usr/local/bin/autopkgserver"])
    def test_main_checks_group_ownership(
        self, mock_dirname, mock_abspath, mock_realpath, mock_stat, mock_geteuid
    ):
        """Should fail if file has wrong group ownership."""
        mock_geteuid.return_value = 0  # Running as root
        mock_abspath.return_value = "/usr/local/bin/autopkgserver"
        mock_realpath.return_value = "/usr/local/bin/autopkgserver"
        ...
        # Mock stat to return wrong group
        mock_stat_result = MagicMock()
        mock_stat_result.st_uid = 0
        mock_stat_result.st_gid = 501  # Not wheel or admin
        mock_stat_result.st_mode = 0o755
        mock_stat.return_value = mock_stat_result
