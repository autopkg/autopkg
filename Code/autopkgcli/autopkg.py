...
def find_recipe(id_or_name: str, skip_overrides: bool = False):
...
def locate_recipe(
    ...
    if not recipe_file:
        # name wasn't a filename. Let's search our map for names and/or identifiers.
...
def load_recipe(
    ...
    # Pre AutoPkg 3.0.0:
    # Here's how this should work, ideally:
    # 1. Search for the recipe based on path, then override, shortname, identifier
    # 2. If it's an override, load up the trust info
    # 3. If there's a ParentRecipe, store this as the child and locate the parent
    # 4. If we located the parent, merge the child and parent together in order,
    # where child recipe values always take precedence
    # 5. Keep doing this until no more ParentRecipes
    # 6. Return the entire recipe as one single big dictionary
    ...
    # Once we have a path to a file:
    # 1. Load the file, attempt to parse into recipe object
    # 2. Is it a valid recipe? (Need to document valid recipe expectations)
    # 3. if not, return None for a recipe object
    # 4. If no valid recipe object, error and fail
    # 5. With a valid recipe object, check for a ParentRecipe
    # 6. Rerun recipe location logic for ParentRecipe
    # 7. When parsing recipe object, prepend Process with processors from each parent, in order
    # 8. Add input variables from parents only if they don't already exist in the child (later recipes always win)
    recipe_file = locate_recipe(
        name,
    ...
    if not recipe_file:
        # TODO: This needs to be improved, because otherwise -d is essentially useless.
        # If we don't find the recipe in the map, we should instead search the provided
        # search directories, and then add it if we find it.
        log(
            "We are not currently searching on-disk for recipes; if you intended to run a specific file not in the "
            "recipe map, please pass in a real file path instead."
        )
        return

    # TODO: This is super dumb. We attempt to read the recipe from the file here,
    # but then immediately after we open the recipe file again to validate it...
    # This should be reversed, and we should validate the file in the process of
    # trying to parse it. ARGH
    recipe = recipe_from_file(recipe_file)

    # Validate the recipe first
    if not valid_recipe_file(recipe_file):
        log_err(f"Invalid recipe: {name}")
        return

    # if recipe_in_override_dir(recipe_file, override_dirs):
    if name in globalRecipeMap["overrides"] or name in globalRecipeMap.get(
...
..adds the autopkg/recipes repo from GitHub."""
    ...
    for repo_url in arguments:
        ...
        if new_recipe_repo_dir:
            ...
            log("Updating recipe map with new repo...")
            calculate_recipe_map(extra_search_dirs=[new_recipe_repo_dir])
...
def repo_delete(argv):
    ...
    # Recalculate the recipe map
    log("Recalculating the recipe map...")
    calculate_recipe_map()
...
def get_info(argv):
    ...
    # Calculate recipe map if we got this
...
def get_trust_info(recipe, search_dirs=None):
    ...
    for p_recipe_path in parent_recipe_paths:
        ...
        p_recipe = load_recipe(p_recipe_path)
...
def run_recipes(argv):
    ...
    # Look for the actual recipes here
    for recipe_path in recipe_paths:
        ...
        log(f"**load_recipe time: {str(end - start)}")
        if not recipe:
