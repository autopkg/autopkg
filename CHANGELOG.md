...
- MunkiInstallsItemsCreator now derives `minimum_os_version` (and reports each created installs item) regardless of whether `faux_root` is set; previously this only worked when `faux_root` had a value. Also fixed the contradictory "as greater than" and "lower... skipping" logs emitted when a subsequent item's `minosversion` equaled the current minimum; equal values now leave `minimum_os_version` alone and log nothing.
- The maintainer scripts no longer catch `BaseException`, so Control-C now interrupts them promptly instead of being swallowed and reported as an API error. GitHub API errors also now carry the underlying reason, and `setup_new_recipe_repo` prints the actual server response body when it isn't valid JSON (previously it printed an empty value).
- AppDmgVersioner now says which app it couldn't read bundle info from when an app's `Info.plist` is missing `CFBundleIdentifier` or `CFBundleShortVersionString`; previously the error was just the bare missing key name. Underlying errors are also now chained in PlistReader, AppDmgVersioner, and PkgCreator, so `-vv` tracebacks show the original cause.
...
- When AutoPkg can't execute git at all (for example, due to a permissions problem), the resulting error now states the underlying reason instead of being blank.
- A fully-qualified shared-processor reference (`RECIPE_ID/Processor`) that can't be resolved to a recipe under the active search directories now fails loudly with an "Unknown processor" error instead of silently falling back to a same-named built-in core processor. Previously, for example, `com.github.homebysix.FindAndReplace/FindAndReplace` would quietly run core `FindAndReplace` (a different implementation) whenever the homebysix recipe was out of scope. Such a reference is now also loaded from the resolved recipe's own directory instead of being searched for across the calling recipe's and parent recipes' directories, so a shared recipe whose processor file is missing or renamed fails the same way rather than falling back to a same-named core processor.
...
> [!WARNING]
> **Custom processor authors**: AutoPkg 3.1.0 plans to include Python 3.12, which removes `distutils`. If your processors still import from `distutils`, they will break on AutoPkg 3.1.0. AutoPkg's `APLooseVersion` is a drop-in replacement for `LooseVersion`; see the [migration guide](https://github.com/autopkg/autopkg/wiki/Migrating-Custom-Processors-to-Python-3#comparing-versions-use-aplooseversion) on the wiki. Pull requests have already been opened against affected processors in the autopkg org.
...
- The autopkginstalld privileged helper daemon now independently validates that package paths are confined to the recipe cache or disk images mounted under `/private/tmp` before invoking the system installer. This check applies regardless of which processor initiates the install.
