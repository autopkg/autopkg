...
- `audit --json` writes one JSON object per audited recipe, each finding tagged with a stable check name (`sensitive_input`, `missing_codesig`, `insecure_protocol`, `path_safety`, `weak_hash`, `non_core_processor`, `modification_processor`) and a severity of `error`, `warning`, or `info`. Findings are ordered most-severe first. `--json` and `--plist` are mutually exclusive; the set of findings `--plist` reports is unchanged.
